...
private fun selectionSort(list: List<Int>): List<Number> {
    ...
    (0..tempList.lastIndex).forEach {
        var index = it
        ((it+1)..tempList.lastIndex).forEach {  next->
            if(tempList[next] < tempList[index])
                index = next

        }
        if( index != it){
            var variable = tempList[it]
            tempList[it] = tempList[index]
            tempList[index] = variable
        }
    }
